...
class WebSockMainHandler(websocket.WebSocketHandler):
        ...
        def confirm_handler():
            ...
            if self.msg.message['type'] == 'get_result':
                ...
                if query.__len__() == 1:
                    ...
                    query.is_valid()
                    query.save()
            elif self.msg.message['type'] == 'xml':
    ...
    @classmethod
    def xmlinfo(cls, method, fp, dev=None, encoding='utf-8', content=None, tag=None):
        seq = hash()
        if dev and type(dev) == list:
            cli = list()
            for i in WebSockMainHandler.clients:
        ...
        reply = ws_protocol.WebsocketProtocol(
            {
                'method': method,
                'seq': seq,
                'callback': None,
...
class XmlInfoHandler(tornado.web.RequestHandler):
    def get(self):
        try:
            dev = self.get_argument('dev').split(';')
        ...
        try:
            ...
            encoding = self.get_argument('encoding')
            if method.lower() == 'set':
                content = self.get_argument('content')
                WebSockMainHandler.xmlinfo(method=method, fp=fp, dev=dev, encoding=encoding, content=content)
            elif method.lower() == 'get':
                tag = self.get_argument('tag')
                WebSockMainHandler.xmlinfo(method=method, fp=fp, dev=dev, encoding=encoding ,tag=tag)
            else:
                self.write('Exception: WrongMethod')
        except tornado.web.MissingArgumentError:
            self.write('Exception: MissingArgument')
